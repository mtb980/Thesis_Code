...
def prior(*theta):
    ...
    p3=scist.uniform.logpdf(theta[2],loc=500,scale=150)
    p4=scist.norm.logpdf((theta[3]),loc=-2,scale=0.1)
...
def f(j):
    ...
    Tm1.append(scist.uniform.rvs(loc=500,scale=150))
    ...
    p1_1=scist.multivariate_normal.logpdf(noisy_TGA,mean=np.array(sol[0]),cov=np.diag(np.ones(N)*(10**sigma[-1])**2))+prior(A1[0],E1[0],Tm1[0],sigma[0],A2[0],E2[0],Tm2[0])
    p1_2=scist.multivariate_normal.logpdf(noisy_DSC,mean=np.array(sol[1]),cov=np.diag(np.ones(N)*((10**sigma[-1])/100)**2))
    p1=p1_1+p1_2
    ps.append(p1)
    pf=1
    for i in range(sample_size-1):
        if i>10000:
            ...
            pf=200
        Tm1t=scist.norm.rvs(loc=Tm1[-1],scale=2/pf,size=1)[0] #log normal variation 
        E1t=scist.norm.rvs(loc=E1[-1],scale=0.05/pf,size=1)[0]
        sigmat=scist.norm.rvs(loc=sigma[-1],scale=0.002,size=1)[0]
        ...
        p2_1=scist.multivariate_normal.logpdf(noisy_TGA,mean=np.array(sol[0]),cov=np.diag(np.ones(N)*(10**sigmat)**2))+prior(A1t,E1t,Tm1t,sigmat,A2t,E2t,Tm2t)
        p2_2=scist.multivariate_normal.logpdf(noisy_DSC,mean=np.array(sol[1]),cov=np.diag(np.ones(N)*((10**sigmat)/100)**2))
        p2=p2_1+p2_2
        p=p2-p1
...
Chain1.to_csv('SIM_output/Chain1.csv',index=False)
Chain2.to_csv('SIM_output/Chain2.csv',index=False)
Chain3.to_csv('SIM_output/Chain3.csv',index=False)
Chain4.to_csv('SIM_output/Chain4.csv',index=False)
