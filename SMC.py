...
def weights(x,data=np.zeros(60),alpha=20):
    ...
    e=sol-data
    w=sum(scist.norm.logpdf(e,scale=sigma))
    return w
...
def MCMC_SMC(theta0,var,likelihood,prior,data,steps=5):
    p=theta0.probs
...
def liklihood(x,data=np.zeros(60),alphas=[5,10,20]):
    ...
    for i in range(len(alphas)):
        ...
        w=w+scist.multivariate_normal.logpdf(data[i],mean=np.array(sol),cov=np.diag(np.ones(len(sol))*sigma))
    return w
...
sample=processData('SMC',Burnin=20000)
...
unique_sample =sample.drop_duplicates()
#unique_sample=small_sample.drop_duplicates()



    
weight_list = Parallel(n_jobs=num_cores)(delayed(weights)(x,data=noisy_TGA5,alpha=alpha5) for x in unique_sample.itertuples())
weight_list2 = Parallel(n_jobs=num_cores)(delayed(weights)(x,data=noisy_TGA20,alpha=alpha20) for x in unique_sample.itertuples())

unique_sample['weights']=np.array(weight_list)+np.array(weight_list2)

#weighted_sample=merge_weights(small_sample, unique_sample)
weighted_sample=merge_weights(sample, unique_sample)

probs=logProb(weighted_sample['weights'].to_numpy())

weighted_sample['probs']=probs

weighted_sample.to_csv('SMC_output/weighted_sample.csv',index=False)


resampled= Parallel(n_jobs=num_cores)(delayed(resample)(weighted_sample,probs) for i in range(k))

resampleddf=pd.DataFrame(resampled)
resampleddf.to_csv('SMC_output/resampled.csv',index=False)

variances=resampleddf[['A','E','sigma']].cov().to_numpy()*2

data=[noisy_TGA5,noisy_TGA10,noisy_TGA20]
samplelist=Parallel(n_jobs=num_cores)(delayed(MCMC_SMC)(theta, var=variances,likelihood=liklihood,prior=prior, data=data) for theta in resampleddf.itertuples(index=False))
