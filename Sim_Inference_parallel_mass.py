...
def prior(*theta):
    ...
    p4=scist.norm.logpdf(math.log10(theta[3]),loc=-2,scale=2)
    p5=scist.uniform.logpdf(theta[6],loc=700,scale=100)
    p=p1+p2+p3+p4+p5
    return p
...
Chain1.to_csv('SIM_mass_output/Chain1.csv',index=False)
Chain2.to_csv('SIM_mass_output/Chain2.csv',index=False)
Chain3.to_csv('SIM_mass_output/Chain3.csv',index=False)
Chain4.to_csv('SIM_mass_output/Chain4.csv',index=False)
